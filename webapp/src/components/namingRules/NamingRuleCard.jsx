...
export default function NamingRuleCard({ rule, onEdit, onDelete, onToggle }) {
  ...
  return (
    ...
    >
      ...
      <div className="mb-4">
        ...
        <Badge color="indigo">{rule.transactionName}</Badge>
      </div>
      ...
      <div className="flex items-center gap-4 text-xs text-gray-600 mb-4 dark:text-gray-300 dark:hover:text-white transition-colors">
        ...
        <span>Matches: {rule.matchCount?.toLocaleString?.() ?? 0}</span>
      </div>
      {/* Patterns Section */}
      <div className="mb-4">
        <button
