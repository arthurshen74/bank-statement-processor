...
import PdfLibToleranceControls from '../components/PdfLibToleranceControls';
import { Heading } from '../ui/heading';
import { getConfig } from '../config';
import { CheckCircleIcon, XCircleIcon } from '@heroicons/react/24/outline';

function ExtractGirokontoStatement() {
  ...
  const handleFileUpload = useCallback(
    async (file) => {
      ...
      formData.append('line_tolerance', tolerances.lineTolerance.toString());
      formData.append('table_padding', tolerances.tablePadding.toString());
      formData.append(
        'intersection_tolerance',
        tolerances.intersectionTolerance.toString()
      );
      formData.append('text_x_tolerance', tolerances.textXTolerance.toString());
      formData.append('text_y_tolerance', tolerances.textYTolerance.toString());

      try {
        const config = getConfig();
        const response = await fetch(
          `${config.pythonApiBaseUrl}/process-pdflib-pdf`,
          {
            method: 'POST',
            body: formData,
          }
  ...
  const handleToleranceChange = (type, value) => {
    setTolerances((prev) => ({
      ...prev,
      [type]: value,
    }));
    ...
    <div className="p-2 max-w-7xl mx-auto space-y-6">
      <div className="mb-6">
        <div className="mb-2 w-full flex justify-between">
          <Heading>Extract Girokonto Statements</Heading>
          {results && (
            <button
              onClick={handleReset}
      ...
      {error && (
        <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-lg dark:bg-red-200/10">
      ...
      {!results && !loading && (
        <div className="space-y-6">
          ...
          <FileUploadWithZip
            ...
            loading={loading}
            zipPatternHelpText='For ZIP files: Only PDFs matching "Ihr Finanzstatus vom" pattern will be processed'
          />
        </div>
      )}
...
export default ExtractGirokontoStatement;
